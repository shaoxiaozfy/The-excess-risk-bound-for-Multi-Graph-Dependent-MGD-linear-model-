...
#    configs['device'] = torch.device('cpu')
    ...
    mean2, std2 = np.mean(test_performances_losses), np.std(test_performances_losses)
    ...
    with open(os.path.join(log_dataset, "test_loss_average.txt"), mode="w+") as f:
        f.write("test_loss mean:" + str(np.round(mean2, 4)) + "\n")
        f.write("test_loss std: " + str(np.round(std2, 4))+ "\n")
        f.write("test_auc mean:" + str(np.round(mean, 4)) + "\n")
        f.write("test_auc std: " + str(np.round(std, 4))+ "\n")
...
if __name__ == '__main__':
    ...
    with open("/data/shaoxiao/Desktop/Multi-graph concentration resources/macro-auc-pytorch-main/running_time_goemotions.txt", mode = "a") as f:
