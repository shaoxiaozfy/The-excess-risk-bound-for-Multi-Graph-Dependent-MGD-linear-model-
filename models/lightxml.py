...
from evaluation.surrogate_loss import SurrogateLossForRankingLoss
...
def get_bert(bert_name):
    if 'roberta' in bert_name:
        ...
        model_config = RobertaConfig.from_pretrained('./downloaded_files/text_data/models/roberta-base')
        model_config.output_hidden_states = True
        bert = RobertaModel.from_pretrained('./downloaded_files/text_data/models/roberta-base', config=model_config)
    elif 'xlnet' in bert_name:
        print('load xlnet-base-cased')
        model_config = XLNetConfig.from_pretrained('./downloaded_files/text_data/models/xlnet-base-cased')
        model_config.output_hidden_states = True
        bert = XLNetModel.from_pretrained('./downloaded_files/text_data/models/xlnet-base-cased', config=model_config)
    else:
        print('load bert-base-uncased')
        model_config = BertConfig.from_pretrained('./downloaded_files/text_data/models/bert-base-uncased')
        model_config.output_hidden_states = True
        bert = BertModel.from_pretrained('./downloaded_files/text_data/models/bert-base-uncased', config=model_config)
    return bert
...
class LightXML(nn.Module):
    def __init__(self, n_labels, group_y=None, bert='bert-base', feature_layers=5, dropout=0.5, update_count=1,
                 candidates_topk=10, use_swa=True, swa_warmup_epoch=10, swa_update_step=200, hidden_dim=300,loss_name="bce"):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids,
        ...
        if self.group_y is None:
            logits = group_logits
            if is_training:
                loss_fn = SurrogateLossForRankingLoss(self.loss_name)
    ...
    def get_accuracy(self, candidates, logits, labels):
        ...
        _, index = torch.sort(logits, 1, descending=True)
        ...
        rankingloss = 0.0
        total = 0
        num_data, num_classes = logits.size()
        ...
        return total, rankingloss, rankingloss, rankingloss
